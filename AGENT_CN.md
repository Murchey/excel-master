...
**自动语言检测**：如果用户使用英文发起对话，Agent 必须**全程使用英文与用户交互**。包括：
- 所有回复和解释
- 错误消息和状态更新
- Web 界面语言（自动切换为英文）
- 结果中的文件名和描述

**Web 界面语言切换**：Web 预览界面支持中英文切换：
- **默认语言**：中文
- **切换方式**：点击右上角地球图标（🌐）
- **语言持久化**：用户的语言偏好保存在浏览器 localStorage 中
- **自动检测**：当 Agent 检测到用户使用英文输入时，应指示 Web 界面切换为英文模式
...
### 3. 增强分析需求（业务方向数据分析与参考文档支持）

**适用场景**：深度数据分析，支持业务方向、参考文档和AI总结
...
1. **获取用户输入**
   ...
   * 分析用户意图和业务分析需求
   * **重要**：确定分析方向（公司/学术）和具体领域（财务/销售/成绩/科研）
   * 询问用户是否有参考文档（MD格式）需要附加
...
3. **读取表格和参考文档**
   ...
   * **读取 `excel_io` Skill**：了解 Excel 文件读取方法
   * 调用 `excel_io.read()` 将 Excel 文件从 temp 目录读取
   * 加载附加的 MD 参考文档
...
5. **打开 WEB 页面确认分析需求（必须）**

   * **读取 `excel_preview` Skill**：了解预览服务的启动方法和操作模式
   * 调用 `excel_preview` Skill 启动 Web 服务，**必须使用 `confirm` 操作模式**
   * **目的**：让用户在网页界面可视化地确认分析需求
   * 用户在网页界面完成以下操作：
     - 查看数据预览和结构信息
     - 选择分析方向（公司/学术）和领域（财务/销售/成绩/科研）
     - 上传或选择参考文档（MD格式）
     - 指定分析目标和输出格式
   * Agent 获取用户确认的需求后，继续后续步骤
   * **此步骤在分析之前执行**，确保分析符合用户需求

6. **执行增强分析**

   * **读取 `excel_analysis` Skill**：了解分析脚本的使用方法和参数说明
   * 调用 `excel_analysis` Skill 进行业务方向数据分析
   * **参数**：
     - `--direction`：分析方向（company/academic/general）
     - `--area`：分析领域（finance/sales/hr/grades/teaching/research）
     - `--reference`：参考MD文档路径
     - `--output`：JSON输出路径
   * 生成包含洞察和建议的综合分析结果
...
7. **保存分析结果**
   ...
   * 分析结果自动保存到工作区JSON文件
   * **读取 `excel_io` Skill**：了解Excel文件保存方法（如需要）
   * 结果保存到 `./excel_output/analysis_result.json`
